...
savePreferences() {
  ...
  const preferences = this.applied
    ...
    .filter((pref): pref is { student_id: number; project_id: number; preference_rank: number } => pref !== null);
  ...
  // savePreferences() {
  //   if (!this.student || !this.student.student_id || !this.applied) {
  //     alert('Student data is missing. Please try again.');
  //     return;
  //   }
  
  //   const preferences = this.applied.map((projectTitle, index) => {
  //     const project = this.allProjects.find(p => p.title === projectTitle);
  //     return project
  //       ? {
  //           student_id: this.student.student_id,
  //           project_id: project.project_id,
  //           preference_rank: index + 1, // Ranking starts from 1
  //         }
  //       : null;
  //   }).filter(pref => pref !== null); // Remove null values
  
  //   console.log("Saving preferences:", preferences);
  
  //   // Send updated preferences to backend
  //   this.applicationService.saveStudentPreferences(preferences).subscribe({
  //     next: (response) => {
  //       console.log('Preferences saved successfully:', response);
  //       alert('Preferences saved successfully!');
  //       this.isEditingPreferences = false; // Hide arrow buttons after saving
  //     },
  //     error: (err) => {
  //       console.error('Error saving preferences:', err);
