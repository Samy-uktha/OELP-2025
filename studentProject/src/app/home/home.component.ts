...
import { Pdf, projApplication, project, Student, preference } from '../interfaces';
...
@Component({
  selector: 'app-home',
  standalone: true,
  imports: [CommonModule, FormsModule, ApplicationsComponent],
    ...
    this.service.getStudent(this.userId).subscribe({
      next: (Data) => {
        ...
        if (this.student.applied) {
          ...
          this.update_applications2(this.student.applied.map(app => app.title));
        }
...
// Fetch preferences
this.appservice.getPref(this.userId).subscribe({
  next: (preferences) => {
    if (preferences && preferences.length > 0) {
      this.applied = preferences.map(pref => {
        const project = this.allProjects.find(p => p.project_id === pref.project_id);
        return project ? { ...pref, title: project.title } : null;
      }).filter(pref => pref !== null).sort((a, b) => a.rank - b.rank);
    }
  },
  ...
  update_applications2(updatedList: string[]) {
    this.applied = updatedList;
  }
  ...
  viewApplication(projectName: string) {
    ...
    if (project) {
      ...
      if (this.applied.includes(project.title)) {
        const appl = this.student.applied?.find(app => app.project_id === project.project_id);
        if (appl) {
          this.application = appl;
          this.applicationsData[project.project_id].bio = appl.bio;
          this.applicationsData[project.project_id].files = appl.docs;
  ...
  savePreferences() {
    // isEditingPreferences = false; // Hide the arrows after saving
  
    // Call the service method to save the updated order
    this.appservice.savePreferences(this.userId, this.applied)
      .subscribe({
  ...
  getBadgeClass() {
    if (!this.application || !this.application.status) return 'badge bg-secondary';
